...
resource "github_actions_secret" "apps_job_ci_oai_key" {
  ...
  plaintext_value = jsondecode(data.aws_secretsmanager_secret_version.apps_job_ci.secret_string)["OPENAI_API_KEY"]
}

resource "github_actions_secret" "apps_job_ci_resend_key" {
  repository      = "job"
  secret_name     = "RESEND_API_KEY"
  plaintext_value = jsondecode(data.aws_secretsmanager_secret_version.apps_job_ci.secret_string)["RESEND_API_KEY"]
}
